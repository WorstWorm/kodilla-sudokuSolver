...
public class PlayerInput {
    public static void getValueFromPlayer(SudokuGame game) {
        System.out.println("enter the data in the format x,y,z where x is the column number, " +
                "y is the row number and z is the value" +
                "\nwhen you want to resolve Sudoku type SUDOKU");
        ...
        while(!inputFinished) {
            ...
            if (input.matches("^[1-9]{1},{1}[1-9]{1},{1}[1-9]{1}")) {
                System.out.println(input.charAt(0) + "," + input.charAt(2) + ","
                        + input.charAt(4) + " - correct input");
